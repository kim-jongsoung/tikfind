    ...
    try {
        ...
        const strategyHint = (() => {
            if (situation === 'quiet') {
                ...
                return myRatio >= 0.5
                    ? '판을 이겼습니다! 다음 판도 이어질 수 있으니 기세를 유지하자고 외쳐주세요.'
                    : '판을 졌지만 다음 판이 있습니다. 역전을 노려볼 수 있습니다.';
            } else if (myRatio <= 0.25) {
                // 크게 뒤질 때 → 미라클 호출
                return '점수 차가 크게 벌어졌습니다. 이럴 때 시청자 중 큰 선물을 한 번에 보내주는 "미라클"이 필요합니다. 미라클을 외쳐서 역전의 기회를 노려보세요!';
            } else if (myRatio <= 0.4) {
                // 뒤처지는 중 → 미라클/스나이퍼
                return '뒤처지고 있습니다. "미라클"을 기다리거나, 마지막에 한 방으로 뒤집어주는 "스나이퍼"를 기다려볼 수 있습니다. 시청자들에게 미라클과 스나이퍼를 외쳐달라고 요청하세요!';
            } else if (myRatio >= 0.75) {
                // 크게 앞설 때 → 마무리 집중
                return '크게 앞서고 있습니다. 상대가 스나이퍼나 미라클로 역전할 수 있으니 방심 금물! 시청자들에게 끝까지 응원을 부탁하세요.';
            } else if (myRatio >= 0.6) {
                // 리드 중 → 유지 전략
                return '리드 중입니다. 상대의 스나이퍼나 미라클을 조심하면서 응원을 이어가야 합니다.';
            } else {
                // 박빙 → 스나이퍼 타이밍
                return '박빙 승부입니다! 마지막 순간 한 방으로 뒤집어주는 "스나이퍼"가 승패를 가릅니다. 시청자들에게 스나이퍼를 외쳐달라고 요청하세요!';
            }
        })();
