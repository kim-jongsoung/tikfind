...
async function importSongs() {
    try {
        console.log('🚀 노래 DB 수집 시작...\n');
        console.log(`📊 총 ${VERIFIED_SONGS.length}곡 저장 예정\n`);
        
        let savedCount = 0;
        let skippedCount = 0;
        
        for (let i = 0; i < VERIFIED_SONGS.length; i++) {
            const song = VERIFIED_SONGS[i];
            const progress = `[${i + 1}/${VERIFIED_SONGS.length}]`;
            ...
            try {
                ...
                console.log(`${progress} ✅ 저장: ${song.title} - ${song.artist}`);
                savedCount++;
                
            } catch (error) {
                console.error(`${progress} ❌ 저장 실패: ${song.title} - ${error.message}`);
            }
        }
        ...
        console.log(`📊 총 처리: ${savedCount + skippedCount}곡`);
