...
if (!YOUTUBE_API_KEY) {
    ...
    console.error('💡 https://console.cloud.google.com/apis/credentials 에서 API Key 발급');
    process.exit(1);
}
...
// 오픈소스 데이터셋 (예시 - 실제 데이터셋으로 교체 가능)
// GitHub에서 K-POP, Billboard 차트 데이터를 가져올 수 있습니다
const DATASET_SONGS = [
    // K-POP 인기곡
...
async function importFromDataset() {
    try {
        console.log('🚀 데이터셋 노래 수집 시작...\n');
        console.log(`📊 총 ${DATASET_SONGS.length}곡 처리 예정`);
        console.log(`⏱️  7분 이상 영상은 자동 제외됩니다.\n`);
        ...
        for (let i = 0; i < DATASET_SONGS.length; i++) {
            const song = DATASET_SONGS[i];
            const progress = `[${i + 1}/${DATASET_SONGS.length}]`;
            
            try {
                // 중복 체크
                const exists = await PopularSong.findOne({ videoId: song.videoId });
                if (exists) {
                    console.log(`${progress} ⏭️  이미 존재: ${song.title} - ${song.artist}`);
                    skippedCount++;
                    continue;
                }
                
                // 영상 길이 확인
                console.log(`${progress} 🔍 길이 확인 중: ${song.title} - ${song.artist}`);
                const duration = await getVideoDuration(song.videoId);
                
                if (duration === null) {
                    console.log(`${progress} ❌ 길이 확인 실패: ${song.title}`);
                    errorCount++;
                    continue;
                }
                ...
                if (duration > MAX_DURATION) {
                    console.log(`${progress} 🚫 너무 긴 영상 (${durationStr}): ${song.title}`);
                    tooLongCount++;
                    continue;
                }
                ...
                console.log(`${progress} ✅ 저장 (${durationStr}): ${song.title} - ${song.artist}`);
                savedCount++;
                ...
                console.error(`${progress} ❌ 처리 실패: ${song.title} - ${error.message}`);
                errorCount++;
            }
        }
        ...
        console.log(`📊 총 처리: ${savedCount + skippedCount + tooLongCount + errorCount}곡`);
